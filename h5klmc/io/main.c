...
int main(int argc, char* argv[]){
	...
	// get file id
	file_id = H5Fcreate("t3.h5", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
 ...
 * * * * */
	...
	FILE* fgout = NULL;
	char* fgout_path = "/work/e05/e05/wkjee/Software/gulpklmc/klmc3_tf_interface.update.12092023/KLMC3.052024/h5klmc/io/A521/gulp_klmc.gout";
	//char* fgout_path = "/work/e05/e05/wkjee/Software/gulpklmc/klmc3_tf_interface.update.12092023/KLMC3.052024/h5klmc/io/A521/gulp_klmc.gou";
	bool isfile = false;
	// filecheck
	isfile = file_exists( fgout_path );

	int line_number = 0;
	char line[2048];

	if( isfile ){
		fgout = fopen(fgout_path,"r");
		fseek( fgout, 0, SEEK_SET );
		// 1. read line
		while( fgets(line,sizeof(line),fgout) != NULL ){

			// DO SOMETHING !!!
			printf("%s",line);
	
			if( strstr(line,"Cartesian lattice vectors") ){
				printf("FOUND %s",line);
			}
		}
		fclose(fgout);
