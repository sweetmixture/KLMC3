...
/* --------------------------------------------
    10.2024 
    GULP algorithm based modes - checker (template)
   -------------------------------------------- */
    if( strcmp(&(tfc->application[0]),TF_APPLICATION_GULP_RQ) == 0 ){    // mode 1 : Random-Quenching
        bapp = true;
        // any possible error must be inspected here
    }
    if( strcmp(&(tfc->application[0]),TF_APPLICATION_GULP_SS) == 0 ){    // mode 2 : Solid-Solution
        bapp = true;
        // any possible error must be inspected here
    }
    if( strcmp(&(tfc->application[0]),TF_APPLICATION_GULP_BH) == 0 ){    // mode 3 : Basin-Hoping
        bapp = true;
        // any possible error must be inspected here
    }
    if( strcmp(&(tfc->application[0]),TF_APPLICATION_GULP_SA) == 0 ){    // mode 4 : Simulated-Annealing
        bapp = true;
        // any possible error must be inspected here
    }
    if( strcmp(&(tfc->application[0]),TF_APPLICATION_GULP_MM) == 0 ){    // mode 5 : Metropolis-MonteCarlo
        bapp = true;
        // any possible error must be inspected here
    }
    if( strcmp(&(tfc->application[0]),TF_APPLICATION_GULP_LM) == 0 ){    // mode 6 : Lid-MonteCarlo
        bapp = true;
        // any possible error must be inspected here
    }
    if( strcmp(&(tfc->application[0]),TF_APPLICATION_GULP_SC) == 0 ){    // mode 7 : Surface-Cluster Translation
        bapp = true;
        // any possible error must be inspected here
    }
    if( strcmp(&(tfc->application[0]),TF_APPLICATION_GULP_GA) == 0 ){    // mode 8 : Genetic-Algorithm
        bapp = true;
        // any possible error must be inspected here
    }
