...
void set_TaskEnvelope(
    ...
    if( strcmp(app,"gulp") == 0 ){
        ...
        task->app_ptr = gulpklmc;                       // set 'gulpklmc()' subroutine - Fortran interface
...
bool ready_input_call_master(
    ...
    const WorkgroupConfig* wgc               // essential for 'master' in order to send tasks, must know base rank of workgroups (base rank of rank(0) of workgroups)
){
    bool berr = true;
    char currentTime[64];
    ...
    * * */
    ...
    strcat(mws.inputsource_dir,"/run");  // 'run' hard-coded for using mode Ready-Input
    ...
    */
     ...
     * * * * */
    for(int n=0;n<master_tag;n++){ // master_tag == last workgroup tag, i.e., looping all workgroups
        ...
        fprintf(iomaster," >>>>>>>> task send (initial) : ready-input mode\n");
    ...
    while( task != NULL ){
        ...
        MPI_Recv(&taskres,sizeof(TaskResultEnvelope),MPI_CHAR,MPI_ANY_SOURCE,MPI_ANY_TAG,*base_comm,&status); // Wait here until receive finish message from any workgroups
        getCurrentDateTime(currentTime);

        /* pre-set for message recving */
        fprintf(iomaster," <<<<<<<< task result recv (task-farming) : ready-input mode\n");
        ...
        MPI_Send(task,sizeof(TaskEnvelope),MPI_CHAR,status.MPI_SOURCE,TASK_WORKTAG,*base_comm); // MPI_Status stauts.MPI_SOURCE : send back to 'recv' source
        getCurrentDateTime(currentTime);
    ...
    /* * *
     * When task ends turns into receive mode only
     * * */
    for(int n=0;n<master_tag;n++){
    ...
    /* * *
     * after receving final message : send termination message
     * * */
    for(int n=0;n<master_tag;n++){
...
void ready_input_call_workgroups(
    ...
    const MPI_Comm* workgroup_comm,        // local  : only known by cpus where they belong (workgroup) to
    const int n_workgroup,                 // global :
    const int workgroup_tag                // local  : only known by cpus where they belong (workgroup) to
){
    bool taskError;
    ...
    int bsize;                             // global  :
    int brank;                             // local   :
    int workgroup_size;                    // local   : only known by cpus where they belong (workgroup) to
    int worker_rank;                       // local   : only known by cpus (workgroup internal rank)
    ...
    const int workgroup_count = n_workgroup - 1; // global :
    const int master_base_rank = bsize - 1;      // global :
